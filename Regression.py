...
for drug, orig_idxs in meta_test.groupby("DRUG_NAME").groups.items():
    ...
    plt.figure(figsize=(12, 0.5 * len(top_idx) + 1.5))
    shap.force_plot(
        expected_value,
        mean_signed[top_idx],
        mean_features[top_idx],
        feature_names=[feature_names[i] for i in top_idx],
        matplotlib=True,
        show=False
    )
    drug_force_path = os.path.join(res_dir, f"force_per_drug_{safe_name(drug)}.png")
    plt.tight_layout()
    plt.savefig(drug_force_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved per-drug force plot: {drug_force_path}")
...
least_force_path = os.path.join(
    res_dir,
    f"force_least_error_{safe_name(pair[0])}_{safe_name(pair[1])}.png"
)
plt.tight_layout()
plt.savefig(least_force_path, dpi=150, bbox_inches="tight")
plt.close()
print(f"Saved least-error force plot: {least_force_path}")
